...
def main():
    ...
    pgm_path = out_dir / "kdu_r1x1_circle_1024x1024.pgm"
    imageio.imwrite(pgm_path, base)
    print(f"Wrote original pgm: {pgm_path}")

    j2c_path = out_dir / "kdu_r1x1.j2c"
    cmd_compress = [
        "kdu_compress",
        "-i",
        str(pgm_path),
    ...
    roundtrip_pgm = out_dir / "kdu_r1x1_roundtrip.pgm"
    cmd_expand_full = [
        "kdu_expand",
        "-i",
        str(j2c_path),
    ...
    print("Computing LL vs subsample images via kdu_expand and NumPy for R1X1...")
    for level in (1, 2, 3, 4):
        ll_path = out_dir / f"kdu_r1x1_ll_codec_L{level}.pgm"
        cmd_ll = [
            "kdu_expand",
            "-i",
            str(j2c_path),
        ...
        sub_path = out_dir / f"kdu_r1x1_ll_subsample_L{level}.pgm"
        diff_path = out_dir / f"kdu_r1x1_ll_diff_L{level}.pgm"
    ...
    print("\nDone. View files in './refdiff_demo' to see differences for R1X1.")
